...
class Game:
    def __init__(self, player_x, player_o, sym, board_size: int, printer):
    ...
    def is_win(self):
        ...
        if row <= col:
            while col - row + self.win_len + offset - 1 <= self.board_size:
                if all(
                    self.board[i + offset][col - row + i + offset] == sym
        ...
        # below the middle
            ...
            while row - col + self.win_len + offset - 1 <= self.board_size:
                if all(
                    self.board[row - col + i + offset][i + offset] == sym
                    for i in range(self.win_len)
                ):
                    return True
                offset += 1
        ...
        if row + col <= self.board_size - 1:
            while row + col - offset >= self.win_len - 1:
                if all(
                    self.board[i + offset][row + col - i - offset] == sym
                    for i in range(self.win_len)
                ):
                    return True
                offset += 1
        # below the middle
            ...
            while row + col + offset < self.board_size * 2 - self.win_len:
                if all(
                    self.board[row + col + 1 - self.board_size + i + offset][
                        self.board_size - 1 - i - offset
                    ]
                    == sym
                    for i in range(self.win_len)
                ):
...
def main():
    ...
    parser.add_argument(
        ...
        default=13,
        nargs="?",
        help="Start date of the report in 'YYYY-MM-DD' format. Defaults to start of previous week",
    )
